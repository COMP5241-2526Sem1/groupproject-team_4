...
"""
	...
	messages = [
		{"role": "system", "content": "You are an expert at creating engaging classroom polls. Always respond with valid JSON only. Do not include any text before or after the JSON."},
		{"role": "user", "content": prompt}
	]
	...
	try:
		...
		# Return a fallback response
		return {
			"poll_name": f"Poll on {topic}",
			"description": f"Class engagement poll on {topic}",
			"duration": 10,
			"questions": [
				{
					"type": "mcq",
					"content": f"What aspect of {topic} interests you most?",
					"points": 0,
					"choices": [
						{"text": "Basic concepts", "correct": false},
						{"text": "Advanced applications", "correct": false},
						{"text": "Real-world examples", "correct": false},
						{"text": "Theoretical background", "correct": false}
					]
				}
			]
		}
	except Exception as e:
		print(f"Error generating poll content: {e}")
		raise
...
"""
	...
	messages = [
		{"role": "system", "content": "You are an expert at creating thought-provoking short answer questions. Always respond with valid JSON only. Do not include any text before or after the JSON."},
		{"role": "user", "content": prompt}
	]
	...
	try:
		...
		# Return a fallback response
		return {
			"name": f"Short Answer on {topic}",
			"description": f"Detailed questions about {topic}",
			"duration": 20,
			"questions": [
				{
					"type": "saq",
					"content": f"Explain the key concepts and principles of {topic}. Provide examples to support your answer.",
					"points": 5,
					"sample_answer": f"A comprehensive explanation of {topic} would include...",
					"grading_criteria": "Look for clear understanding of concepts, use of examples, logical structure"
				}
			]
		}
	except Exception as e:
		print(f"Error generating short answer content: {e}")
		raise
