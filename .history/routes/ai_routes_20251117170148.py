...
def generate_short_answer():
    """Generate short answer questions using AI and save to database"""
    try:
        data = request.get_json()
        short_answer_id = data.get('short_answer_id')
        topic = data.get('topic', '')
        teaching_materials = data.get('teaching_materials', '')
        num_questions = data.get('num_questions', 3)
        ...
        if not short_answer_id:
            return jsonify({"success": False, "error": "Short answer ID is required"}), 400
        
        # Get the short answer object
        short_answer = ShortAnswer.query.get(short_answer_id)
        if not short_answer:
            return jsonify({"success": False, "error": "Short answer not found"}), 404
        ...
        ai_content = generate_short_answer_content(topic, teaching_materials, num_questions)
        
        # Create questions from AI-generated content
        for q_data in ai_content.get('questions', []):
            question = Question(
        ...
        db.session.commit()
        return jsonify({"success": True, "message": "Questions generated and saved successfully"})
        
    except Exception as e:
        db.session.rollback()
        return jsonify({"success": False, "error": f"Error generating short answer questions: {str(e)}"}), 500
