...
def short_answer_detail(course_code, short_answer_id):
    """View short answer activity details (for students) - shows all student answers"""
    if 'user_id' not in session:
        return redirect(url_for('auth.login'))
    ...
    # Get all submissions for this short answer (all students)
...
@short_answer_bp.route('/teacher/course/<course_code>/short-answer/<int:short_answer_id>/results')
def short_answer_results(course_code, short_answer_id):
    """Display results of short answer activity for teacher - shows student responses"""
    if 'user_id' not in session:
        return redirect(url_for('auth.login'))
    ...
    return render_template('short_answer_results.html',
                         course=course,
                         short_answer=short_answer,
                         submissions=submissions)
...
@short_answer_bp.route('/api/short-answer/<int:short_answer_id>/results')
def get_short_answer_results(short_answer_id):
    """API endpoint to get short answer results data (for dynamic updates)"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'error': 'Not authenticated'}), 401
    ...
    try:
        ...
        # Get all submissions with their responses
        submissions = Submission.query.filter_by(short_answer_id=short_answer_id).all()
        ...
        for submission in submissions:
            responses = []
            for question in short_answer.questions:
                qr = QuestionResponse.query.filter_by(
                    submission_id=submission.id,
                ...
                responses.append({
                    ...
                    'text_answer': qr.text_answer if qr else None,
                    'points': qr.points if qr else 0
                })
        ...
        return jsonify({
            'success': True,
            'short_answer_id': short_answer_id,
            'name': short_answer.name,
            'description': short_answer.description,
            'total_submissions': len(submissions),
            'submissions': submissions_data,
            'questions': [{'id': q.id, 'content': q.content} for q in short_answer.questions]
        })
