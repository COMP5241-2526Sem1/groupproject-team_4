...
def test_with_alice():
    """Test quiz info page with alice_johnson login"""
    
    # Login first
    login_url = "http://localhost:5000/login"
    login_data = {
        "username": "alice_johnson",
        "password": "password",  # Found this from the SQL hash
        "role": "student"
    }
    
    session = requests.Session()
    
    print("Attempting login as alice_johnson...")
    ...
    if login_response.status_code == 200:
        ...
        if quiz_response.status_code == 200:
            ...
            if 'View Results' in content:
                ...
                with open('quiz_info_alice.html', 'w', encoding='utf-8') as f:
                    f.write(content)
                print("Saved response to quiz_info_alice.html")
